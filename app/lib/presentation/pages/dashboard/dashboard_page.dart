...
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/dialogs/add_item_dialog.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/add_item_button_widget.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/calories_chart.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/dashboard_app_bar.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/macros_chart.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/tab_all_day_widget.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/tab_bar_widget.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/tab_breakfast_widget.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/tab_dinner_widget.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/tab_lunch_widget.dart';
import 'package:flutter_nutrition_ai_demo/presentation/pages/dashboard/widgets/tab_snack_widget.dart';
...
class _DashboardPageState extends State<DashboardPage> with SingleTickerProviderStateMixin {
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: MacrosChart(
                                          key: UniqueKey(),
                                          data: macroData,
                                          userProfile: _userSession.userProfile,
                                        ),
                                      ),
                                    ],
                                  ),
      ...
      builder: (BuildContext context) => CupertinoActionSheet(
        ...
        actions: [
          CupertinoActionSheetAction(
            onPressed: () {
              context.pushNamed(Routes.progressPage);
          ...
          CupertinoActionSheetAction(
            onPressed: () async {
              bool? hasChanges = await context.pushNamed(Routes.favouritesPage);
              if(hasChanges ?? false) {
                _doApiCall();
              }
              if(context.mounted) {
    ...
    AddItemDialog.show(
      ...
      onTapItem: (item) async {
        ...
        if (item == context.localization?.quickFoodScan) {
          bool? hasAdded = await context.pushNamed(Routes.quickFoodScanPage, extra: {AppConstants.dateTime: _selectedDate});
          if (hasAdded ?? false) {
            _doApiCall();
          }
        } else if (item == context.localization?.multiFoodScan) {
          bool? hasAdded = await context.pushNamed(Routes.multiFoodScanPage, extra: {AppConstants.dateTime: _selectedDate});
          if (hasAdded ?? false) {
            _doApiCall();
          }
        } else if (item == context.localization?.byTextSearch) {
          PassioIDAndName? data = await context.pushNamed(Routes.foodSearchPage);
          if (data != null) {
  ...
  Future<void> _onEditItem(int index, FoodRecord? data) async {
    ...
    (String? action, FoodRecord? foodRecord)? result =
        await context.pushNamed(Routes.editFoodPage, extra: {AppConstants.index: index, AppConstants.data: data});
      ...
      if (foodRecord != null) {
        ...
        if (context.mounted) {
          ...
          if (result.$1 == context.localization?.save) {
            ...
            _foodRecordsList[index] = foodRecord;
            // Once list is updated successfully then update the graph.
            _updateGraph();
            // Firing bloc event to update the food record in local DB.
            _bloc.add(DoFoodUpdateEvent(data: foodRecord));
          } else if (result.$1 == context.localization?.favorites) {
            // Showing success message for saving favorite in DB.
            context.showSnackbar(text: context.localization?.favoriteSuccessMessage);
            // Firing favorite event to store the data in DB.
  ...
  void _handleFoodInsertSuccessState({required FoodInsertSuccessState state}) {
    _foodRecordsList.insert(0, state.data);
    _updateGraph();
  }
