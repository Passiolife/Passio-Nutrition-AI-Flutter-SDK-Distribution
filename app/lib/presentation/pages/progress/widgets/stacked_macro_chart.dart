...
class StackedMacroChart extends StatelessWidget {
  ...
  double get _totalCarbsValue =>
      timeLog.isNotEmpty ? timeLog.map((e) => e.foodRecords.isNotEmpty ? e.totalCarbs : 0.0).reduce((value, element) => math.max(value, element)) : 0;

  double get _totalProteinValue => timeLog.isNotEmpty
      ? timeLog.map((e) => e.foodRecords.isNotEmpty ? e.totalProteins : 0.0).reduce((value, element) => math.max(value, element))
      : 0;

  double get _totalFatValue =>
      timeLog.isNotEmpty ? timeLog.map((e) => e.foodRecords.isNotEmpty ? e.totalFat : 0.0).reduce((value, element) => math.max(value, element)) : 0;

  double get _maxValue => math.max(_totalCarbsValue + _totalProteinValue + _totalFatValue, 5);

  List<ChartData> get chartData {
    return List.generate(timeLog.length, (index) {
      final data = timeLog.elementAt(index);
      return ChartData(
        x: timeLog.length > 7 ? data.dateTime.day.toString() : data.dateTime.format(format10),
        yCarbs: data.foodRecords.isNotEmpty ? data.totalCarbs.roundNumber(1) : 0,
        yProtein: data.foodRecords.isNotEmpty ? data.totalProteins.roundNumber(1) : 0,
          ...
          legendItemBuilder: (String name, dynamic series, dynamic point, int index) {
            if (series is StackedColumnSeries) {
              return Wrap(
                ...
                children: [
                  (index == 0) ? Dimens.w34.horizontalSpace : Dimens.w8.horizontalSpace,
                  Container(
                    color: series.color,
                    width: Dimens.r10,
                    height: Dimens.r10,
